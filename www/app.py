...
def datetime_filter(t):
	...
	if delta < 86400:	# 24 * 60 * 60 = 86400
		return u'%s小时前' % (delta // 3600)
	if delta < 604800:	# 7 * 24 * 60 * 60 = 604800
		return u'%s天前' % (delta // 86400)
	dt = datetime.fromtimestamp(t)
	return u'%s年%s月%s日' % (dt.year, dt.month, dt.day)

async def init(loop):
	await orm.create_pool(loop=loop, host=srv_ip, port=3306, user='www-data', password='www-data', db='awesome')
